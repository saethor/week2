...
module.exports=function(injected){
    ...
    const incomingCommandDispatcher = IncomingSocketMessageDispatcher(inject({
        ...
        fs:fs,
        incomingMessageLogFile: './server-received-commands.log'
    }));


    const socketIoEventPort = OutgoingSocketIoMessagePort(inject({
        io:io,
        messageRouter:eventRouter,
        fs: fs,
        outgoingMessageLogFile: './server-issued-events.log'
    }));
