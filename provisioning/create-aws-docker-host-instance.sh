...
if [ ! -e ${INSTANCE_DIR}/security-group-name.txt ]; then
...
if [ ! -e ${INSTANCE_DIR}/instance-public-name.txt ]; then
...
aws ec2 authorize-security-group-ingress --group-name ${SECURITY_GROUP_NAME} --protocol tcp --port 8080
